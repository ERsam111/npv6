...
export function ResultsPanel({
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      const updatedDcs = await Promise.all(dcs.map(async dc => {
        try {
          const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
          const SUPABASE_KEY = import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY;
          const response = await fetch(`${SUPABASE_URL}/functions/v1/find-nearest-city`, {
            method: 'POST',
            headers: {
  ...
  const calculateCustomerTransportationCost = (customer: Customer, dc: DistributionCenter): number => {
    ...
    const demandInCostUnit = convertDemand(customer.demand, customer.unitOfMeasure, settings.costUnit, product);
      ...
      <Card className="shadow-lg">
        ...
        <div className="space-y-2">
          ...
          <ResponsiveContainer width="100%" height={Math.max(200, dcs.length * 40)}>
            <BarChart data={demandBySite} layout="vertical" margin={{
                left: 20,
                right: 50
              }}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
              <XAxis type="number" className="text-xs" tick={{
              ...
              <YAxis type="category" dataKey="site" className="text-xs" tick={{
                  fill: 'hsl(var(--muted-foreground))'
                }} width={80} />
              ...
              <Bar dataKey="percentage" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} label={{
                  position: 'right',
                  fill: 'hsl(var(--foreground))',
                  fontSize: 12,
                  formatter: (value: number) => `${value.toFixed(1)}%`
                }}>
                {demandBySite.map((entry, index) => <Cell key={`cell-${index}`} fill={`hsl(var(--primary) / ${1 - index * 0.05})`} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>

        {costBreakdown && <div className="p-4 bg-primary/5 rounded-lg space-y-3 border border-primary/20">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="space-y-1">
                ...
                <p className="font-semibold text-lg">${costBreakdown.transportationCost.toLocaleString(undefined, {
                    maximumFractionDigits: 0
                  })}</p>
              </div>
              <div className="space-y-1">
                <p className="text-muted-foreground">Facility Cost:</p>
                <p className="font-semibold text-lg">${costBreakdown.facilityCost.toLocaleString(undefined, {
                    maximumFractionDigits: 0
                  })}</p>
              </div>
            </div>
            <div className="pt-2 border-t border-primary/20">
              <div className="flex justify-between items-center">
                ...
                <p className="font-bold text-xl text-primary">${costBreakdown.totalCost.toLocaleString(undefined, {
                    maximumFractionDigits: 0
                  })}</p>
              </div>
        ...
        {warnings.length > 0 && <div className="space-y-2">
            {warnings.map((warning, index) => <Alert key={index} variant="destructive">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {loadingCities && <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Loader2 className="h-3 w-3 animate-spin" />
                Finding nearest cities...
              </div>}
          </div>
          <div className="max-h-[400px] overflow-y-auto space-y-3">
            {dcsWithCities.map(dc => <div key={dc.id} className="p-4 bg-muted rounded-lg space-y-2 border border-border">
                <div className="flex items-center justify-between">
                ...
                {dc.nearestCity && <div className="bg-primary/10 rounded px-3 py-2 border border-primary/20">
                ...
                {/* Demand by Product */}
                {dc.assignedCustomers.length > 0 && <div className="text-xs space-y-1">
                    <p className="text-muted-foreground">Demand by Product:</p>
                    {Object.entries(dc.assignedCustomers.reduce((acc, customer) => {
                    ...
                    demand: number;
                    unit: string;
                  }>)).map(([product, data]) => <p key={product} className="pl-2">
                ...
                <div className="pt-2 border-t border-border/50">
                  ...
                  <div className="space-y-2">
                    {dc.assignedCustomers.map(customer => {
                      ...
                      return <div key={customer.id} className="text-xs pl-2 bg-background p-2 rounded space-y-1">
